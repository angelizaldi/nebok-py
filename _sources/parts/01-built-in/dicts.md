...
El tipo de dato `dict` corresponde a una colección de objetos identificados por una llave. Algunas características de las diccionarios, son:
- Utilizan un formato _key-value_ para almacenar valores.
- Es mutable: Se puede modificar una vez creado.
- Sus elementos no están ordenados, es decir, no se puede acceder a sus elementos por medio de índices, pero sí se puede puede hacer _subsetting_ por medio de las llaves.
...
**1.** Poner los elementos en un par _key: value_, separados por coma, dentro de llaves `{}`:
```python
...
:::{tip}
Si se tiene dos _iterable_ que se quiere que funcionen como _keys_ y como _values_ se podría usar la función `zip()` y `dict()` para crear el diccionario: <br> `my_dict = dict(zip(iterable1, iterable2))`
:::
...
- `dict.update()`: Agregar elementos desde otro objeto.
...
- `dict.pop()`: Remover un elemento específico.
- `dict.popitem()`: Remover y retornar un elemento de un índice en específico.
- `dict.clear()`: Remover todos los elementos.
...
```python
# X es un diccionario y 'key' es una llave válida
X['key']
```

```{note}
Otras formas de acceder a valores con métodos son:
- `dict.get()`: Acceder a un elemento con una llave en específico.
- `dict.values()`: Acceder a todos los valores.
- `dict.keys()`: Acceder a todas las llaves.
- `dict.items()`: Accedes a todos los pares _key-value_.
...
for key, value in X.items():
    # for body
```
- En todos los ejemplos anteriores _key_ y _value_ son nombres opcionales.
...
```python
# Sintaxis básica con una variable
X = {key_expression : value_expression for i in collection}

# Sintaxis básica con dos variables
X = {key_expression : value_expression for key, value in collection}
```

Los dict comprehesions también pueden tener condicionales, tanto en la parte de _expression_ como en la parte de _collection_, de manera que solo en ciertos elementos se evalúe la expresión o de que dependiendo del resultado de la _expression_ se ponga un valor u otro. Sintaxis:
...
X = [key_expression : value_expression for key, value in collection if condition]
```

**En la _expression_**: En este caso los valores de _x_ dependerán del resultado de _condition_.
```python
# list comprehension con condicional en expression
X = [key_expression : value_expression_true if condition else value_expression_false for key, value for i in collection]
```
...
```python
# List comprehession anidado
X = {key_expression: value_expression for i in collection1 for j in collection2}
```

**Ejemplo**: 
Definir un diccionario cuyos _kesy_ sean los número del 1 al 6, y cuyos _values_ sean los cuadrados de esos números.
```{code-cell} ipython3
# Definir el diccionario
cuadrados = {x: x*x for x in range(1, 6)} 
print(cuadrados)
```
...
* - [items](https://docs.python.org/3/library/stdtypes.html#dict.items)()
  - Devuelve una _view_ de los elementos del diccionario (clave, valor).
* - [keys](https://docs.python.org/3/library/stdtypes.html#dict.keys)()
  - Devuelve una _view_ de las llaves del diccionario.
* - [values](https://docs.python.org/3/library/stdtypes.html#dict.values)()
  - Devuelve una _view_ de los valores del diccionario.
```
...
* - [update](https://docs.python.org/3/library/stdtypes.html#dict.update)([other])
  - Actualiza el diccionario con los pares llave/valor de _other_, sobrescribiendo llaves existentes. Realiza la operación in-place.
```
...
* - [pop](https://docs.python.org/3/library/stdtypes.html#dict.pop)(key[, default])
  - Si la llave está en el diccionario, elimina la llave y retorna su valor, de lo contrario, retorna _default_. Si no se proporciona el valor _default_ y la llave no está en el diccionario, se genera un `KeyError`.
...
* - [fromkeys](https://docs.python.org/3/library/stdtypes.html#dict.fromkeys)(iterable[, value])
  - Crea un nuevo diccionario con llaves del _iterable_ y los valores se establecen como _value_.
