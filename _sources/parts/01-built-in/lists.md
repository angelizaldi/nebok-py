...
El tipo de dato `list` corresponde a una colección mutable de objetos. Algunas características de las listas son:
...
# Agregar elemento z a W
W = W + [z]

# Convertir objeto Z a lista y concatenarlo a W
W = W + list(Z)
```
- Si solo se quiere agregar un elemento que no es una lista encerrar entre corchetes
- Si se quiere concatenar otro objeto que no es una lista, convertirlo primero a lista con la función `list()` y después concatenarlo, siempre y cuando la conversión sea posible, en caso contrario usar el método anterior.

```{note}
Otras formas de agregar elementos usando métodos son:
- `list.insert()`: Insertar algún elemento en un índice en específico sin remplazarlo (recorriendo el resto de los elementos a la derecha).
- `list.append()`: Insertar algún elemento al final de la lista usar el método .
...
- `list.remove()`: Remover un elemento específico usar el método .
- `list.pop()`: Remover un elemento de un índice en específico usar el método .
...
X[i:j] = [val1, val2, ...]
```
- La lista con los nuevos valores debe de tener el mismo tamaño que `X[i:j]`.
...
- En `X[i:j]`:
    - Si se omite el índice _i_ significa desde el inicio de la _lista_.
    - Si se omite el índice _j_ significa hasta el final de la _lista_.
    - Si se omiten ambos significa toda la _lista_. 
...
- Desde la posición _i_ hasta el final de la lista: <br>`X[i:]`
- Toda la lista: <br> `X[:]`
...
Son listas que se crean a partir procesos iterativos con la estructura `for`.
```python
# Uso básico de list comprehession
...
```python
# Equivalencia en un for loop de un list comprehession
X = []
for i in collection:
    X.append(expression)
```

Las _list comprehension_ también pueden tener condicionales, tanto en la parte de _expression_ como en la parte de _collection_, de manera que solo en ciertos elementos se evalúe la expresión o de que dependiendo del resultado de la _expression_ se ponga un valor u otro. Sintaxis:
...
**En la _expression_**: En este caso el elemento de _x_ dependerá del resultado de _condition_.
```python
...
**Ejemplo**: 
Encontrar los elementos al cuadrado de una lista numérica, solo si el número es par.
```{code-cell} ipython3
# Definir un iterable
numeros = range(1, 11)
...
Tener en cuenta que los métodos generalmente se aplican sobre un objeto de tipo `list`, por ejemplo, si _x_ es `list`, entonces se utiliza <code>X.<i>method_name</i></code>. Sin embargo es posible usar la lista como argumento de <code>list.<i>method_name</i></code>. Por ejemplo.
...
* - [insert](https://docs.python.org/3/library/stdtypes.html#mutable-sequence-types)(i, x)
  - Inserta _x_ en una lista en el índice dado por _i_, recorriendo el resto de los elementos a la derecha.
```
...
* - [count](https://docs.python.org/3/library/stdtypes.html#common-sequence-operations)(x)
  - Retorna el número total de ocurrencias de _x_ en la lista.
* - [index](https://docs.python.org/3/library/stdtypes.html#common-sequence-operations)(x[, i[, j]])
  - Indice de la primera aparición de _x_ en la lista (en o después índice _i_ y antes del índice _j_). Retorna `ValueError` si _x_ no está en la lista. 
```
...
- `sorted()`: Ordenar el contenido de una lista.
- `reversed()`: Invertir el orden actual de una lista.

:::{tip}
Para más funciones revisar las secciones {ref}`func-iterables` y {ref}`func-iterables-numbericos`.
:::